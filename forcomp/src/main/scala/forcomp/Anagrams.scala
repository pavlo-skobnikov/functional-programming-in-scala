...
object Anagrams extends AnagramsInterface:
  ...
  extension (original: List[Char])
    /** Converts a list of characters into a histogram of characters i.e. a map
      * from characters to their frequency in the list.
      */
    private def toOccurrences: Occurrences = original
      .map(_.toLower)
      .groupBy(identity)
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(word: Word): Occurrences = word.toList.toOccurrences

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(sentence: Sentence): Occurrences =
    sentence.mkString.toList.toOccurrences
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.groupMap(wordOccurrences(_))(identity)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    val possibleOccurences = occurrences.map(charAndFreq =>
      val sequenceOfPossibleCharacterOccurences =
        for i <- 1 to (charAndFreq._2)
        yield (charAndFreq._1, i)

      sequenceOfPossibleCharacterOccurences.toList
    )

    possibleOccurences.foldRight(List[Occurrences](Nil))(
      (leftOccurence, rightOccurrence) =>
        val combinationsOfLeftAndRight = for
          i <- leftOccurence
          j <- rightOccurrence
        yield (i :: j)

        rightOccurrence ++ combinationsOfLeftAndRight
    )
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val subtrahendMap = y.toMap

    x.foldLeft(List[(Char, Int)]())((acc, minuendCharAndFreq) =>
      val (minuendChar, minuendFreq) = minuendCharAndFreq
      val subtrahendFreq = subtrahendMap.getOrElse(minuendCharAndFreq._1, 0)

      val newFreq = minuendFreq - subtrahendFreq

      if newFreq > 0 then acc :+ (minuendChar, newFreq) else acc
    ).sortBy(_._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def inner(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(List())
        ...
        for
          occurrencePermutation <- combinations(
            occurrences
          ) filter dictionaryByOccurrences.contains
          word <- dictionaryByOccurrences(occurrencePermutation)
          sentence <- inner(subtract(occurrences, occurrencePermutation))
        yield word :: sentence
