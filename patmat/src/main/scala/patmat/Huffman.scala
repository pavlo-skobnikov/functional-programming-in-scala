...
trait Huffman extends HuffmanInterface:
  ...
  def times(chars: List[Char]): List[(Char, Int)] =
    def incFrequency(freqOpt: Option[Int]): Option[Int] = freqOpt match
      case None            => Some(1)
      case Some(frequency) => Some(frequency + 1)

    @tailrec
    def timesLoop(chars: List[Char], acc: Map[Char, Int]): Map[Char, Int] =
      chars match
        case Nil => acc
        case head :: tail =>
          timesLoop(tail, acc.updatedWith(head)(incFrequency))

    timesLoop(chars, Map.empty).toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    def lessThanFrequencyComparator(
        left: (Char, Int),
        right: (Char, Int)
    ): Boolean =
      if left._2 == right._2 then left._1 < right._1 else left._2 < right._2

    freqs
      .sortWith(lessThanFrequencyComparator)
      .map(charAndFreq => Leaf(charAndFreq._1, charAndFreq._2))
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] =
    @tailrec
    def combineLoop(
        treesToProcess: List[CodeTree],
        acc: TreeSet[CodeTree]
    ): TreeSet[CodeTree] = treesToProcess match
      case Nil                    => acc
      case head :: Nil            => acc + head
      case first :: second :: Nil => acc + makeCodeTree(first, second)
      case first :: second :: tail =>
        combineLoop(tail, acc + makeCodeTree(first, second))

    combineLoop(trees, TreeSet.empty(Ordering.by[CodeTree, Int](weight))).toList
  ...
  def until(
    ...
    @tailrec
    def untilLoop(treesToMerge: List[CodeTree]): List[CodeTree] =
      if done(treesToMerge)
      then treesToMerge
      else untilLoop(merge(treesToMerge))
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    val charFrequencies = times(chars)
    val orderedLeafList = makeOrderedLeafList(charFrequencies)
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] =
    ...
    def decodeLoop(
        walkedTree: CodeTree,
        bitsToDecode: List[Bit],
        acc: List[Char]
    ): List[Char] = bitsToDecode match
      case Nil =>
        walkedTree match
          case Leaf(char, weight)               => acc :+ char
          case Fork(left, right, chars, weight) => acc
      case headBit :: nextBit =>
        walkedTree match
          case Leaf(char, weight) => decodeLoop(tree, bitsToDecode, acc :+ char)
          case Fork(left, right, chars, weight) =>
            if headBit == 0
            then decodeLoop(left, nextBit, acc)
            else decodeLoop(right, nextBit, acc)

    decodeLoop(tree, bits, List.empty)
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] =
    def subTreeContainsChar(subTree: CodeTree, char: Char): Boolean =
      subTree match
        case Leaf(leafChar, weight)           => leafChar == char
        case Fork(left, right, chars, weight) => chars.contains(char)

    @tailrec
    def encodeSingleCharLoop(
        walkedTree: CodeTree,
        char: Char,
        acc: List[Bit]
    ): List[Bit] = walkedTree match
      case Leaf(leafChar, weight) => acc
      case Fork(left, right, chars, weight) =>
        if subTreeContainsChar(left, char)
        then encodeSingleCharLoop(left, char, acc :+ 0)
        else encodeSingleCharLoop(right, char, acc :+ 1)

    def encodeLoop(text: List[Char], acc: List[Bit]): List[Bit] =
      text match
        case Nil => acc
        case head :: next =>
          encodeLoop(next, acc ::: encodeSingleCharLoop(tree, head, List.empty))

    encodeLoop(text, List.empty)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match
    case Nil =>
      throw IllegalArgumentException(
        s"Char '$char' not found in table: '$table'"
      )
    case head :: next =>
      if head._1 == char
      then head._2
  ...
  def convert(tree: CodeTree): CodeTable =
    def convertLoop(treeChars: List[Char], acc: CodeTable): CodeTable =
      treeChars match
        case Nil => acc
        case head :: next =>
          val bits = encode(tree)(List(head))

          convertLoop(next, acc :+ (head, bits))

    convertLoop(chars(tree), List.empty)
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = (a ::: b)
    .sortWith((left, right) => left._2.length < right._2.length)
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] =
    ...
    val codeBitsCurriedFn = codeBits(codeTable)

    def quickEncodeLoop(text: List[Char], acc: List[Bit]): List[Bit] =
      text match
        case Nil => acc
        case head :: next =>
          quickEncodeLoop(next, acc ::: codeBitsCurriedFn(head))
    quickEncodeLoop(text, List.empty)
